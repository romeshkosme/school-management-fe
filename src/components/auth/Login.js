...
function Login(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!tnc) {
      ...
      if (login && login.data && login.data["success"]) {
        navigate("/dashboard" || "/login");
        props.setUser(login.data["response"].accessToken);
        getFirebaseBackend().signInWithCustomToken(
          login.data["response"].accessToken
        );
      }
    } else {
      setState({});
